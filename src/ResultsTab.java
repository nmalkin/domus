...
public class ResultsTab extends JPanel {
    ...
    public ResultsTab() {
        ...
        _noPeopleLabel.setPreferredSize(new Dimension(
        ...
        // add listeners to Group and State for updating results based on
        // certain user interactions
        State.getInstance().getGroup().addGroupStateChangeListener(
                new GroupStateChangeListener());
        State.getInstance().addSelectedHouseChangeListener(
                new SelectedHouseChangeListener());
        State.getInstance().setSelectedProbabilityModelChangeListener(
                new SelectedProbabilityModelChangeListener());
        State.getInstance().setSelectedYearsChangeListener(
                new SelectedYearsChangeListener());
    }

    /**
     * Updates the results lists based on the boolean argument. If it's true,
     * then fetch new results from the database. If it's false, then just update
     * the probabilities for each result.
     * 
     * @param updateEverything
     *            , fetch new results if true, update probabilities if false
     */
    ...
    private class GroupStateChangeListener implements ChangeListener {
        ...
        public void stateChanged(ChangeEvent e) {
            if (!((Group.GroupChangeEvent) e).getUpdateType())
                ...
                // update what should be updated based on the GroupChangeEvent
                // updateType
                updateResults(((Group.GroupChangeEvent) e).getUpdateType());
        }
    ...
    /**
     * Listens for the selected house to change. If the results tab is visible
     * and this happens, then the results should be updated.
     * 
     * @author jswarren
     */
    ...
    /**
     * Listens for the selected probability model to change. If the results tab
     * is visible and this happens, then the results should be updated.
     * 
     * @author jswarren
     */
    private class SelectedProbabilityModelChangeListener implements
            ChangeListener {
    ...
    /**
     * Listens for the selected years to change. If the results tab is visible
     * and this happens, then the results should be updated.
     * 
     * @author jswarren
     */
