...
/**
 * A generic class to represent an AccordionList, that is a list with expandable
 * elements. In this class, list items that can be expanded (and thus contain
 * other list items) will be referred to as tabs. List items inside of tabs will
 * simply be referred to as items. List items must extend JComponent, and
 * implement AccordionItem as well as Comparable.
 * 
 * @author jswarren
 */
public class AccordionList<K extends JComponent & AccordionItem, V extends JComponent & AccordionItem>
        extends JPanel {
    ...
    private AccordionList(int width, int height, int headerHeight) {
        ...
        _scroller.setBorder(BorderFactory.createMatteBorder(1, 0, 0, 0,
                Color.BLACK));
        this.add(_scroller);
    }
    ...
    public static <K extends JComponent & AccordionItem, V extends JComponent & AccordionItem> AccordionList<K, V> create(
            int width, int height, int headerHeight) {
    ...
    public void addTab(K tab) {
        ...
        _listsPanel.setPreferredSize(new Dimension(size.width, size.height
                + tab.getPreferredSize().height));
        _listsPanel.setSize(new Dimension(size.width, size.height
                + tab.getPreferredSize().height));
        _listsPanel.setMaximumSize(new Dimension(size.width, size.height
                + tab.getPreferredSize().height));
        tab.setVisible(true);
        _lastTab = tab;
    }
    ...
    /**
     * Sets the currently selected item. Only one should be selected at once.
     */
        ...
        public void componentResized(ComponentEvent e) {
            ...
            if (visible != _scrollbarVisible) {
                ...
                if (_scrollbarVisible)
                    bottom = 1;
                _scroller.setBorder(BorderFactory.createMatteBorder(1, 0,
