...
public class Person extends CanvasComponent {
    ...
    /**
     * Returns the appropriate alpha value for this person based on its
     * location.
     * 
     * If the person is over the trash can, it will return an alpha value
     * corresponding to transparency.
     * 
     * @return
     */
    private float getAlphaValue() {
        if (this.overTrashIcon()
                || // if hovering over trash, draw this transparently
                _subgroup != null && _subgroup.overTrashIcon()
                || // if my subgroup is over the trash, also draw transparently
                _subgroup != null && _subgroup.getHouse() != null
                && _subgroup.getHouse().overTrashIcon()) // same for house
        {
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (alpha < 1) {
            ...
            java.awt.image.RescaleOp rop = new java.awt.image.RescaleOp(scales,
                    offsets, null);

            java.awt.Graphics2D g2 = (java.awt.Graphics2D) g;
            g2.drawImage((java.awt.image.BufferedImage) _gender.getImage(),
                    rop, 0, 0);
        } else {
        ...
        public void mouseClicked(MouseEvent e) {
            if (e.getClickCount() == 2) {
                String newName = (String) JOptionPane.showInputDialog(
                        (java.awt.Component) e.getSource(), "Enter a name:",
                        "Domus", JOptionPane.QUESTION_MESSAGE, null, null,
                        _name);
