...
public class SubGroup extends CanvasComponent implements Iterable<Person>,
        Comparable<SubGroup> {
     ...
     * 
     * @param p
     * @return true if the person was found and removed, or false if the person
     *         wasn't found
     */
    ...
    public boolean sameGender() {
        ...
        Gender g;
        if (isEmpty()) {
            return true; // if the room is empty, return true. this is
                         // meaningless.
        } else {
    ...
    protected void updatePeoplePositions() {
        ...
        for (Person p : _people) {
            p.setPosition(getPosition().x + horizontalOffset, getPosition().y
                    + Constants.SUBGROUP_PADDING);

            horizontalOffset += p.getPreferredSize().width
                    + Constants.SUBGROUP_PADDING;
        }
    }
     ...
     * 
     * If the subgroup is over the trash icon, it will return a transparent
     * version of its color.
     * 
     * @return
     */
    private Color getColor() {
        if (this.overTrashIcon() || // if hovering over trash, draw this
                                    // transparently
                _house != null && _house.overTrashIcon()) // if my house is over
                                                          // the trash, also
                                                          // draw transparently
        {
    ...
    protected void paintComponent(Graphics g) {
        ...
        g2.fill(new RoundRectangle2D.Double(Constants.INSET, Constants.INSET,
                width - 2 * Constants.INSET, height - 2 * Constants.INSET, 10,
                10));
    }
    ...
    /**
     * Compares SubGroups based on creation order: a SubGroup that was created
     * earlier is "smaller" than a SubGroup that was created later.
     */
    @Override
