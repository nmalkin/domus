...
import domus.data.Gender;
import domus.data.LocationPreference;
import domus.gui.util.GraphicsSupport;
...
public class Canvas extends JLayeredPane {
    ...
    // image positions
    private final int _new_male_x_position; // note: x position for icons does
                                            // not change
    ...
    private Canvas() {
        ...
        Dimension d = new Dimension(Constants.CANVAS_WIDTH,
        ...
        _new_male_x_position = (0 + Constants.SIDEBAR_WIDTH) / 2
                - Gender.MALE.getImageDimension().width / 2;
        _new_female_x_position = (0 + Constants.SIDEBAR_WIDTH) / 2
                - Gender.FEMALE.getImageDimension().width / 2;
        _trash_x_position = (0 + Constants.SIDEBAR_WIDTH) / 2
                - Constants.TRASH_WIDTH / 2;
    }

    /**
     * Redraws the Canvas and all components based on the objects currently in
     * State.
     */
     ...
     * @return
     */
    private static boolean intersecting(DraggablePositionableComponent a,
            DraggablePositionableComponent b) {
        return GraphicsSupport.intersectionAreaFraction(a.getRectangle(), b
                .getRectangle()) > Constants.INTERSECTION_FRACTION;
    }
     ...
     * 
     * This entails checking what subgroup the person was dropped inside: if
     * it's the subgroup it was already in, nothing's changed; if it's a new
     * subgroup, it is added there and removed from the old one.
     * 
     * @param person
     */
    public void dropPerson(Person person) {
        ...
        outer: for (House h : State.getInstance().getGroup()) {
            if (intersecting(person, h)) { // person is inside this house
                for (SubGroup s : h) {
                    if (intersecting(person, s)) { // person is inside this
                                                   // subgroup
                        newSubGroup = s;
                        break outer;
                    }
        ...
        if (currentSubGroup != null) {
            if (currentSubGroup == newSubGroup) { // ...unless he's staying in
                                                  // the same subgroup
                currentSubGroup.updatePeoplePositions(); // move the person pack
                                                         // to his/her spot
            ...
            removeIfEmpty(currentSubGroup); // (may require cleaning up
                                            // subgroup/house, if they have been
                                            // left empty)
        ...
        if (newSubGroup == null) { // no subgroup was found for this person.
                                   // create a new one.
            newSubGroup = new SubGroup();

            // place the new subgroup wherever the person is right now
            newSubGroup.setPosition(person.getPosition().x
                    - Constants.SUBGROUP_PADDING, person.getPosition().y
                    - Constants.SUBGROUP_PADDING);
            ...
            /*
             * when being placed, the subgroup could have been merged with a
             * different one, and therefore deleted. to account for this
             * possibility, let's make sure newSubGroup really is the subgroup
             * that the person has been added to.
             */
            newSubGroup = person.getSubGroup();

            // if an entirely new house was created for this person,
            // get the preferences from the old house and copy them to the new
            // house
            if (currentHouse != null && newSubGroup.getOccupancy() == 1
                    && newSubGroup.getHouse().numberOfSubGroups() == 1) {
                newSubGroup.getHouse().setLocationPreference(
                        currentHouse.getLocationPreference());
            }
        } else {
    ...
    public void dropSubGroup(SubGroup subgroup) {
        if (tryRemove(subgroup)) { // if the subgroup is over the trash, remove
                                   // them
            repaint();
            return;
        }
    ...
    public void placeSubGroup(SubGroup subgroup) {
        ...
        for (House h : State.getInstance().getGroup()) {
            if (intersecting(subgroup, h)) { // subgroup is inside this house
                ...
                for (SubGroup s : h) {
                    if (subgroup != s && intersecting(subgroup, s)) { // subgroup
                                                                      // is
                                                                      // inside
                                                                      // this
                                                                      // subgroup
                ...
                // subgroup is inside house, but not intersecting any existing
                // subgroups
                // add it to the house.
                newHouse = h;
                break;
        ...
        if (newHouse == null) {
            ...
            // set the position of the new house to wherever the subgroup is
            // right now
            newHouse.setPosition(subgroup.getPosition().x
                    - Constants.HOUSE_PADDING, subgroup.getPosition().y
                    - Constants.HOUSE_PADDING);
            ...
            // house
            if (currentHouse != null) {
                newHouse.setLocationPreference(currentHouse
                        .getLocationPreference());
            }
        } else if (newHouse == currentHouse) {
            currentHouse.updateSubGroupPositions(); // move subgroup to its
                                                    // default location in house
            this.repaint();
            return;
        }
     ...
     * 
     * Specifically, Checks if it is over the trash can (if it is, it is
     * removed). Places the house on the canvas (@see {@link #placeHouse(House)}
     * )
     * 
     * @param house
     */
     ...
     * 
     * Specifically, Checks if the house is over the sidebar (if it is, moves it
     * off). Checks if it is intersecting any house (if it is, they are merged).
     * 
     * @param house
     */
    protected void placeHouse(House house) {
        ...
        for (House h : State.getInstance().getGroup()) {
            if (house != h && intersecting(house, h)) { // the houses are
                                                        // intersecting
                // merge them!
                ...
                for (SubGroup s : tmp) {
                    h.addSubGroup(s); // this also removes them from the current
                                      // house
                }

                // add the old house's location preferences the new house
                // (the new house's location preferences are the union of the
                // two old ones)
                LocationPreference newHousePreference = h
                        .getLocationPreference();
    ...
    private void mergeSubGroups(SubGroup a, SubGroup b) {
        ...
        /*
         * unfortunately, we can't just do: for(Person p : a) { b.addPerson(p);
         * } because this creates a ConcurrentModificationException. So the
         * workaround is to copy the people to a temporary container, and delete
         * from there. This is hacky. It would be nice to avoid this. (TODO)
         */
        ...
        for (Person p : tmp) {
            b.addPerson(p);
            // the addPerson code also removes the current person from his/her
            // current subgroup (a)
        }
    ...
    @Deprecated
    private boolean isDraggablePositionableComponentAt(
            DraggablePositionableComponent c, int x, int y) {
        // System.out.println("Is " +
        // x + " within [" + c.getPosition().x + "," + (c.getPosition().x +
        // c.getWidth()) + "] and " +
        // y + " within [" + c.getPosition().y + "," + (c.getPosition().y +
        // c.getHeight()) + "]?");

        return x >= c.getPosition().x && x <= c.getPosition().x + c.getWidth()
                && y >= c.getPosition().y
     ...
     * 
     * @param p
     *            the person to (possibly) be removed
     * @return true if the person was removed, false if the person was not
     *         removed
     */
     ...
     * 
     * @param s
     *            the subgroup to (possibly) be removed
     * @return true if the subgroup was removed, false if the subgroup was not
     *         removed
     */
     ...
     * 
     * @param h
     *            the house to (possibly) be removed
    ...
    public boolean overTrashIcon(DraggablePositionableComponent c) {
        return GraphicsSupport.intersectionAreaFraction(c.getRectangle(),
                getTrashRectangle()) > Constants.INTERSECTION_FRACTION;
    }
    ...
    private Rectangle getTrashRectangle() {
        return new Rectangle(_trash_x_position, _trash_y_position,
                Constants.TRASH_WIDTH, Constants.TRASH_HEIGHT);
    }

    private void updateIconPositions() {
        _new_male_y_position = this.getHeight() / 4
                - Gender.MALE.getImageDimension().height / 2;
        _new_female_y_position = this.getHeight() / 4 * 2
                - Gender.FEMALE.getImageDimension().height / 2;
        _trash_y_position = this.getHeight() / 4 * 3 - Constants.TRASH_HEIGHT
                / 2;
    }
    ...
    protected void paintComponent(Graphics g) {
        ...
        g.drawLine(Constants.SIDEBAR_WIDTH, 0, Constants.SIDEBAR_WIDTH, this
                .getHeight());
        g.setColor(Constants.SIDEBAR_COLOR);
        g.fillRect(0, 0, Constants.SIDEBAR_WIDTH, this.getHeight());

        // watermark
        g.drawImage(_domusImage, (this.getWidth() + Constants.SIDEBAR_WIDTH)
                / 2 - _domusImage.getWidth(null) / 2,
                (this.getHeight() - _domusImage.getHeight(null)) / 2, null);
        ...
        g.drawImage(Gender.MALE.getImage(), _new_male_x_position,
                _new_male_y_position, null);
        ...
        g.drawImage(Gender.FEMALE.getImage(), _new_female_x_position,
                _new_female_y_position, null);
    ...
    private class AddPersonListener extends MouseAdapter {
        ...
        /**
         * Given a position on the Canvas, returns null if the position is not
         * over any "New Person" button; otherwise, returns the gender of that
         * new person.
         * 
         * @param x
         * @param y
        ...
        private Gender overNewPersonIcon(int x, int y) {
            if (x >= _new_male_x_position
                    && x <= _new_male_x_position
                            + Gender.MALE.getImageDimension().width
                    && y >= _new_male_y_position
                    && y <= _new_male_y_position
                            + Gender.MALE.getImageDimension().height) {
                return Gender.MALE;
            } else if (x >= _new_female_x_position
                    && x <= _new_female_x_position
                            + Gender.FEMALE.getImageDimension().width
                    && y >= _new_female_y_position
                    && y <= _new_female_y_position
                            + Gender.FEMALE.getImageDimension().height) {
        ...
        public void mousePressed(MouseEvent e) {
            ...
            if (newPersonGender != null) {
                // before adding a person, make sure the group hasn't reached
                // its size limit
                if (State.getInstance().getGroup().numberOfPeople() < Constants.GROUP_SIZE_LIMIT) {
                    Person newPerson = new Person(
                            Constants.NEW_PERSON_DEFAULT_NAME, newPersonGender);

                    switch (newPersonGender) {
                    case MALE:
                        newPerson.setPosition(_new_male_x_position,
                                _new_male_y_position);
                        break;
                    case FEMALE:
                        newPerson.setPosition(_new_female_x_position,
                                _new_female_y_position);
                        break;
                    }
                    ...
                    JOptionPane.showMessageDialog(Canvas.this,
                            "You've reached the limit on group size.", "Domus",
