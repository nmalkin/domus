...
public class House extends CanvasComponent implements Iterable<SubGroup>,
        Comparable<House> {
     ...
     * 
     * @param s
     * @return true if the subgroup was found and removed, or false if the
     *         subgroup wasn't found
     */
    ...
    protected void updateSubGroupPositions() {
        ...
        for (SubGroup s : _subgroups) {
            ...
            // make sure the position where we're placing this subgroup is not
            // off the screen
            if (xPosition + s.getWidth() + Constants.HOUSE_PADDING > Canvas
                    .getInstance().getWidth()
                    && horizontalOffset > Constants.HOUSE_PADDING) // do not
                                                                   // allow
                                                                   // wrapping
                                                                   // on the
                                                                   // first
                                                                   ...
                                                                   // row
            {
                ...
                // NOTE: we are assuming here that all SubGroups have the same
                // height as the current one
            ...
            horizontalOffset += s.getWidth() + Constants.HOUSE_PADDING; // offset
                                                                        // the
                                                                        // position
                                                                        // of
                                                                        // the
            ...
            _height = Math.max(_height, verticalOffset + s.getHeight()
                    + Constants.HOUSE_PADDING); // update the height if
                                                // necessary
        }
    ...
    public boolean contains(int x, int y) {
        ...
        Rectangle rectangle = new Rectangle(Constants.INSET,
                Constants.HOUSE_ROOF_HEIGHT + Constants.INSET, width - 2
                        * Constants.INSET, height - Constants.HOUSE_ROOF_HEIGHT
                        - 2 * Constants.INSET);

        int[] xPoints = { Constants.INSET, width / 2, width - Constants.INSET };
        int[] yPoints = { Constants.HOUSE_ROOF_HEIGHT + Constants.INSET,
     ...
     * 
     * If the house is over the trash icon, it will return a transparent version
     * of its color.
     * 
     * @return
     */
    private Color getColor() {
        if (this.overTrashIcon()) { // if hovering over trash, draw this
                                    // transparently
    ...
    private Color getBorderColor() {
        if (Canvas.getInstance().overTrashIcon(this)) { // if hovering over
                                                        // trash, draw this
                                                        // transparently
    ...
    protected void paintComponent(Graphics g) {
        ...
        updatePosition(); // updates component dimensions (in addition to
                          // position)

        // draw box representing the subgroup
        java.awt.Shape houseBox = new Rectangle(Constants.INSET,
                Constants.HOUSE_ROOF_HEIGHT + Constants.INSET, width - 2
                        * Constants.INSET, height - Constants.HOUSE_ROOF_HEIGHT
                        - 2 * Constants.INSET);
        ...
        if (this == State.getInstance().getSelectedHouse()) {
            ...
            g2.setStroke(new java.awt.BasicStroke(
                    Constants.SELECTED_HOUSE_BORDER_WIDTH,
            ...
            int houseRightBoundary = Constants.INSET + width - 2
                    * Constants.INSET;
            int houseTopBoundary = Constants.HOUSE_ROOF_HEIGHT
                    + Constants.INSET;
            int houseBottomBoundary = Constants.HOUSE_ROOF_HEIGHT
                    + Constants.INSET + height - Constants.HOUSE_ROOF_HEIGHT
                    - 2 * Constants.INSET;

            g.drawLine(houseRightBoundary, houseTopBoundary,
                    houseRightBoundary, houseBottomBoundary);
            g.drawLine(houseRightBoundary, houseBottomBoundary,
                    houseLeftBoundary, houseBottomBoundary);
            g.drawLine(houseLeftBoundary, houseBottomBoundary,
                    houseLeftBoundary, houseTopBoundary);
        }
    }

    /**
     * Compares Houses based on creation order: a House that was created earlier
     * is "smaller" than a House that was created later.
     */
    @Override
