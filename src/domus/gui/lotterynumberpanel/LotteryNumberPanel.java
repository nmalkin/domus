package domus.gui.lotterynumberpanel;
...
/**
 * The LotteryNumberPanel allows the user to input their lottery number, either
 * directly (through the slider) or, indirectly, by having the program estimate
 * it based on their semester level and optimism.
 * 
 * @author nmalkin, jswarren, mmschnei
 * 
 */
public class LotteryNumberPanel extends JPanel implements ChangeListener,
        ActionListener, AncestorListener {

    private static ImageIcon _happyImage = new ImageIcon(Constants.HAPPY_FILE,
            "yay!");
    private static ImageIcon _okayImage = new ImageIcon(Constants.OKAY_FILE,
            "eh");
    private static ImageIcon _sadImage = new ImageIcon(Constants.SAD_FILE,
            "booo");
    private static ImageIcon _sadImageDim = new ImageIcon(
            Constants.SAD_FILE_DIM, "booo");
    private static ImageIcon _happyImageDim = new ImageIcon(
            Constants.HAPPY_FILE_DIM, "yay!");
    private static ImageIcon _okayImageDim = new ImageIcon(
            Constants.OKAY_FILE_DIM, "eh");
    ...
    public LotteryNumberPanel() {
        // set panel size
        Dimension size = new Dimension(Constants.LOTTERY_PANEL_WIDTH,
                Constants.LOTTERY_PANEL_HEIGHT);
        ...
        // slider
        _numberSlider = new JSlider(JSlider.VERTICAL, 1, Database
                .getMaxLotteryNumber(), Constants.DEFAULT_LOTTERY_NUMBER);
        ...
        // TODO: rather than ints, have these be objects, so we can include a
        // string description (e.g., "sophomore") as well
        ...
        int semesterLevel = Database
                .semesterFromLotteryNumber(Constants.DEFAULT_LOTTERY_NUMBER);
        ...
        lotteryNumTitle
                .setFont(Constants.DOMUS_FONT.deriveFont(Font.PLAIN, 16));
        ...
        semesterLevelTitle.setFont(Constants.DOMUS_FONT.deriveFont(Font.PLAIN,
                16));
        ...
        optimismLevelTitle.setFont(Constants.DOMUS_FONT.deriveFont(Font.PLAIN,
                16));
    ...
    public void updateSliderFromSemester() {
        int lotteryNumber = Database.predictLotteryNumber(_semesterLevelBox
                .getSelectedIndex() + 3, _selectedOptimism);
    ...
    public void stateChanged(ChangeEvent e) {
        if (e.getSource() != _numberSlider)
            throw new IllegalArgumentException("expecting event from slider");
        ...
        if (_numberSlider.hasFocus()) {
            ...
            int semesterLevel = Database
                    .semesterFromLotteryNumber(lotteryNumber);
            int semesterIndex = semesterLevel - 3; // the first semester
                                                   // displayed in the combobox
                                                   // is 3
            // TODO: have a better system; probably affected by whether the
            // semesterLevel stays an int
            // (see todo above)
    ...
    /**
     * Given an optimism level, selects the appropriate button (and deselects
     * the others).
     * 
     * @param optimism
     *            the optimism level
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() != _semesterLevelBox)
            throw new IllegalArgumentException("expecting event from combobox");
        ...
        if (_semesterLevelBox.hasFocus()) {
            ...
            int lotteryNumber = Database.predictLotteryNumber(semester,
                    _selectedOptimism);
