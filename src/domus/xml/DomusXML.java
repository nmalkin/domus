...
public class DomusXML {
    ...
    /**
     * Given a filename, writes an HTML document, representing a transform of
     * the State XML file, to this file.
     * 
     * @param filename
     * @throws IOException
     *             if there was a problem writing to the file
     * @throws TransformerException
     *             if there was a problem applying the XSL transformation
     */
    public static void writeHTML(File file) throws IOException,
            TransformerException {
        Document document = makeXMLDocument();
        Document htmlDocument = styleDocument(document,
                Constants.XML_TRANFORM_FILE);
    ...
    private static void writeDocument(Document document, File file)
        ...
        XMLWriter fileWriter = new XMLWriter(outStream, OutputFormat
                .createPrettyPrint()); // output using "pretty" formatting
    ...
    public static Document makeXMLDocument() {
        ...
        group.addAttribute("lottery_number", State.getInstance().getGroup()
                .getLotteryNumber()
                + "");
    ...
    /**
     * Given a filename, reads in the provided XML document, and copies the
     * settings into State.
     * 
     * @param filename
     * @throws IOException
     ...
     *             if there was a problem with the structure of the document
     */
    public static void readXML(File file) throws IOException, DocumentException {
        ...
        // we will create a new Group and new RoomLists for the data we're
        // reading in,
        // and then copy them to State at the end, when we've ascertained the
        // lack of issues
        ...
        if (root.getQName().getName() != "domus") {
            ...
            throw new DocumentException(
                    "unrecognized file format version number");
        }
        ...
        try {
            ...
            if (sophomore.equals("yes")) {
                ...
                throw new DocumentException(
                        "invalid value for attribute \"sophomore\"");
            }
            ...
            for (Object o : group.elements("house")) {
                ...
                for (Object o2 : subgroups.elements("subgroup")) {
                    ...
                    for (Object o3 : subgroup.elements("person")) {
                        ...
                        if (gender.equals("male")) {
                            ...
                            throw new DocumentException(
                                    "unrecognized string for gender");
                        }
                ...
                for (Object o4 : buildings.elements("building")) {
                    ...
                    Dorm dorm = Database.getDorm(building
                            .attributeValue("name"));
                    if (dorm == null) {
            ...
            for (Object o5 : choices.elements("list")) {
                ...
                for (Object o6 : list.elements("room")) {
                    ...
                    Dorm dorm = Database.getDorm(room
                    ...
                    Database.updateRoomData(newRoom); // get lottery
                                                      // results/coefficients if
                                                      // necessary
                    ResultsListItem newItem = new ResultsListItem(newRoom, null);
                    rl.add(newItem);
                    newRoom.addToRoomList(rl); // TODO: is it really necessary
                                               // to call both?
                }
        ...
        if (newGroup.numberOfPeople() > Constants.GROUP_SIZE_LIMIT) {
            throw new DocumentException("input document contains more than "
                    + Constants.GROUP_SIZE_LIMIT
                    + " people, which is not allowed");
        }
    ...
    /**
     * Returns the int value of the attribute of the Element identified by the
     * given name.
     * 
     * @param element
     *            the element whose attribute you're looking for
     * @param attributeName
     ...
     */
    protected static int getIntAttribute(Element element, String attributeName)
            throws DocumentException {
        if (element == null)
        ...
        if (attribute == null) { // non-existing attribute
            throw new DocumentException("element " + element.getName()
        ...
        try {
            ...
            throw new DocumentException("expected attribute " + attributeName
                    + " to be a number. it wasn't");
        }
        return i;
    }

    /**
     * Given a dom4j Document, styles it according to the XSL stylesheet in the
     * specified file.
     * 
     ...
     * @throws TransformerException
     */
    private static Document styleDocument(Document document, String stylesheet)
            throws TransformerException {
