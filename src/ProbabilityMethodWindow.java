...
public class ProbabilityMethodWindow extends JFrame {
    ...
    protected ProbabilityMethodWindow() {
        ...
        for (int year : State.getInstance().getIgnoredYears())
        ...
        for (int year : State.getInstance().getYears())
        ...
        _regressionMethod = new JRadioButton(
                "Calculate probability using regression (best results)");
        _regressionMethod.addItemListener(new MethodListener());
        this.add(_regressionMethod);

        _stupidMethod = new JRadioButton(
                "Calculate probability using past results for lottery number");
        ...
        if (State.getInstance().useRegressionProbability()) {
            ...
            _stupidMethod.setSelected(true);
    ...
    public void sort(DefaultListModel listHandler) {
        if (listHandler.size() != 1 && listHandler.size() != 0) {
            ...
            for (int i = 0; i < years.length; i++)
                years[i] = (Integer) listHandler.elementAt(i);
            years = quicksort(years, 0, years.length - 1);

            listHandler.clear();
            for (int i = 0; i < years.length; i++)
                listHandler.insertElementAt(years[i], i);
        }
    }
    ...
    private Integer[] quicksort(Integer[] years, int first, int last) {
        ...
        while (i <= j) {
            while (years[i] < middle)
                i++;
            while (years[j] > middle)
                j--;
        ...
        if (first < j)
            quicksort(years, first, j);
        if (i < last)
            quicksort(years, i, last);
