...
public class ResultsPanel extends JPanel implements Runnable {
    ...
    private ImageIcon _leftButtonIcon = new ImageIcon(Constants.LEFT_ARROW,
            "scroll left");
    private ImageIcon _rightButtonIcon = new ImageIcon(Constants.RIGHT_ARROW,
            "scroll right");
    ...
    public ResultsPanel() {
        super();
        this.setPreferredSize(new Dimension(Constants.RESULTS_PANEL_WIDTH,
                Constants.RESULTS_PANEL_HEIGHT));
        _resultsPanel = new JPanel();
        _resultsPanel.setPreferredSize(new Dimension(0,
                Constants.RESULTS_PANEL_HEIGHT));
        _resultsPanel.setSize(new Dimension(0, Constants.RESULTS_PANEL_HEIGHT));
        _resultsPanel.setLayout(new BoxLayout(_resultsPanel,
                BoxLayout.LINE_AXIS));
        _resultsPanel.setBorder(Constants.EMPTY_BORDER);
        _scroller = new JScrollPane(_resultsPanel,
                JScrollPane.VERTICAL_SCROLLBAR_NEVER,
                JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
        _scroller
                .setPreferredSize(new Dimension(0,
                        Constants.RESULTS_PANEL_HEIGHT
                                + _scroller.getHorizontalScrollBar()
    ...
    public void updateResultsLists(boolean updateEverything) {
        ...
        SetMultimap<SubGroup, Dorm> dormMap = TreeMultimap.create(Ordering
                .natural(), new DormComparator());
        ...
        for (House h : State.getInstance().getGroup()) {
            for (SubGroup sg : h) {
                // if there is not an existing AccordionList for this SubGroup,
                // create a new one
                AccordionList<ResultsListTab, ResultsListItem> list = null;
                if ((list = _listsMap.get(sg)) == null) {
                    list = AccordionList.create(Constants.RESULTS_LIST_WIDTH,
                            Constants.RESULTS_LIST_HEIGHT,
                            Constants.RESULTS_HEADER_HEIGHT);
                ...
                if (h.getLocationPreference().size() != 0) {
                    ...
                    if (dormMap.keySet().contains(sg)) {
                        // set the header of the AccordionList (Person icons and
                        // names)
                        list.setHeader(headerTop);

                        // get the old ResultsListTabs from the AccordionList
                        // (could be empty if new list)
                        Collection<ResultsListTab> tabs = list.getTabs();

                        // remove any ResultsListTabs which don't have a
                        // corresponding Dorm for this SubGroup
                        for (Iterator<ResultsListTab> iter = tabs.iterator(); iter
                                .hasNext();) {
                        ...
                        for (Dorm d : dormMap.get(sg)) {
                            // if there is an existing ResultsListTab for this
                            // Dorm, update it
                            ...
                            if (tab != null) {
                                ...
                                // create a new ResultsListTab and add all the
                                // rooms to it
                            ...
                            // make sure tabs display the average of their
                            // components' probabilities
                            tab.updateProbabilities();

                            // make sure the ResultsListTab is displaying all of
                            // the list labels it should be
                            tab.validateListLabels();
                        }
                    } else {
                        JLabel message = new JLabel(
                                "Sorry! We found no available results.");
                        message.setHorizontalAlignment(JLabel.CENTER);
                        message.setBorder(new MatteBorder(1, 1, 0, 1,
                                Color.BLACK));
        ...
        int displayLists = Math
                .min(numLists, Constants.RESULTS_LISTS_DISPLAYED);
        Dimension size = new Dimension(numLists * Constants.RESULTS_LIST_WIDTH
                + ((numLists - 1) * Constants.RESULTS_PANEL_HORIZONTAL_GAP),
                Constants.RESULTS_PANEL_HEIGHT);
        ...
        size = new Dimension(
                displayLists
                        * Constants.RESULTS_LIST_WIDTH
                        + ((displayLists - 1) * Constants.RESULTS_PANEL_HORIZONTAL_GAP),
                _scroller.getSize().height);
    ...
    /**
     * Creates a header for the AccordionList. The header consists of a Person
     * icon for each person in the SubGroup, as well as the names of the Person
     * objects, if they have been named by the user.
     * 
     * @param sg
     *            , the SubGroup for which to create a header
    ...
    private JPanel createListHeader(SubGroup sg) {
        ...
        panel.setPreferredSize(new Dimension(Constants.RESULTS_LIST_WIDTH,
                Constants.RESULTS_HEADER_HEIGHT));
        panel.setSize(new Dimension(Constants.RESULTS_LIST_WIDTH,
                Constants.RESULTS_HEADER_HEIGHT));
        ...
        ImageIcon man = new ImageIcon(Gender.MALE.getImage().getScaledInstance(
                -1, 30, Image.SCALE_SMOOTH));
        ImageIcon woman = new ImageIcon(Gender.FEMALE.getImage()
                .getScaledInstance(-1, 30, Image.SCALE_SMOOTH));

        // determine how much width is left on the header after the icons are
        // added
        int width = Constants.RESULTS_LIST_WIDTH
                - (man.getIconWidth() * sg.getOccupancy());
        ...
        // if the names label is not empty, set the tooltip in case it is too
        // long
    ...
    private ResultsListTab containsDormTab(Collection<ResultsListTab> tabs,
            Dorm d) {
    ...
    /**
     * Performs an intersection on the results already in the ResultsListTab,
     * and those returned from the database for the corresponding Drom.
     * 
     * @param sg
     *            , the SubGroup to which the Dorm and ResultsListTab being
     *            merged belong
     * @param d
     *            , the Dorm to merge with the ResultsListTab
     * @param list
     *            , the AccordionList the ResultsListTab belongs too
     * @param tab
     *            , the ResultsListTab to merge with the Dorm
     */
    private void intersectResultsWithTab(SubGroup sg, Dorm d,
            AccordionList<ResultsListTab, ResultsListItem> list,
            ResultsListTab tab) {
        ...
        // if the ResultsListTab is empty, add all of the Rooms in the results
        // to it
        // then return
        else {
     ...
     * 
     * @param list
     *            , the AccordionList to which to add the ResultsListItem
     * @param tab
     *            , the ResultsListTab to which to add the ResultsListItem
     * @param r
     *            , the Room to add to the ResultsListTab
     */
    private void addListItem(
            AccordionList<ResultsListTab, ResultsListItem> list,
            ResultsListTab tab, Room r) {
    ...
    private void updateProbabilities() {
        for (AccordionList<ResultsListTab, ResultsListItem> list : _listsMap
                .values()) {
    ...
    public void run() {
        ...
        while (_scrollLeft || _scrollRight) {
            if (_scrollOnce
                    && count > (Constants.RESULTS_LIST_WIDTH / _scrollWidth)) {
                break;
            }
            ...
            if (_scrollRight) {
                ...
                if (r.x + r.width + _scrollWidth >= _resultsPanel.getWidth()) {
                    view.setViewPosition(new Point(_resultsPanel.getWidth()
                            - r.width, r.y));
                    _scrollRight = false;
                    _scrollLeft = false;
                    break;
    ...
    /**
     * Listens for mouseClicks on the scroll buttons (if they are visible) and
     * scrolls the panel appropriately.
     * 
     * @author jswarren
     */
