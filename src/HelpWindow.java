...
public class HelpWindow extends JFrame {
    ...
    public HelpWindow() {
        ...
        tabbedPane.addTab("Preferences",
                newImageTab(Constants.HELP_SCREEN_PREFERENCE_FILE));
        tabbedPane.addTab("Results",
                newImageTab(Constants.HELP_SCREEN_RESULTS_FILE));
        ...
        this.addComponentListener(new ComponentAdapter() {
            public void componentResized(ComponentEvent e) {
                // resizing the components, as below, would be nice, but it is
                // too slow
                // for(ImageIcon screen : _screens) {
                // Image scaledImage = screen.getImage().getScaledInstance(-1,
                // HelpWindow.this.getHeight(), Image.SCALE_SMOOTH);
    ...
    private JPanel newImageTab(String imageFileName) {
        ...
        // scale image
        Image scaledImage = screen.getImage().getScaledInstance(-1,
                Constants.HELP_SCREEN_HEIGHT, Image.SCALE_SMOOTH);
        screen.setImage(scaledImage);
        ...
        // set the tab's size
        tab.setPreferredSize(new Dimension(screen.getIconWidth(), screen
                .getIconHeight()));
