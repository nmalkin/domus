...
public class ListPanel extends JPanel {
    ...
    private static ImageIcon _removeIcon = new ImageIcon(Constants.REMOVE_FILE,
            "remove from list");
    ...
    public ListPanel(RoomList list) {
        ...
        label
                .setFont(new Font(_font.getFontName(), Font.BOLD, _font
                        .getSize()));
        ...
        panel.setBorder(new EmptyBorder(0, Constants.INSET + 1, 0,
                Constants.INSET + 1));
        this.add(panel);
        _panel = new JList();
        ...
        _scroller.setBounds(0, panel.getHeight(), _listWidth, _listHeight
                - _itemHeight);
        this.add(_scroller);
        updateList();
    }
    ...
    /**
     * Updates the list by removing all components and then adding all existing
     * ones.
     */
    public void updateList() {
        ...
        for (ResultsListItem r : _list) {
            ...
            _panel.setPreferredSize(new Dimension(size.width, size.height
                    + r.getSize().height));
            _panel.setSize(new Dimension(size.width, size.height
                    + r.getSize().height));
            _panel.add(r);
            r.updateProbability();
