...
public class ResultsListTab extends JPanel implements AccordionItem {
    ...
    private static ImageIcon _openIcon = new ImageIcon(Constants.OPEN_FILE,
            "open results list");
    private static ImageIcon _closedIcon = new ImageIcon(Constants.CLOSED_FILE,
            "closed results list");
    private static ImageIcon _addToListIcon = new ImageIcon(Constants.ADD_FILE,
            "add to list");
    ...
    ResultsListTab(Dorm dorm, SubGroup sg,
        ...
        _parentList = list;
        this.setLayout(null);
        this.setPreferredSize(new Dimension(Constants.RESULTS_LIST_WIDTH,
                Constants.RESULTS_LIST_TAB_HEIGHT));
        this.setSize(new Dimension(Constants.RESULTS_LIST_WIDTH,
                Constants.RESULTS_LIST_TAB_HEIGHT));
        _isOpen = false;
        _fullWidth = true;
        ...
        _tab.setBounds(0, 0, Constants.RESULTS_LIST_WIDTH,
                Constants.RESULTS_LIST_TAB_HEIGHT);
        _tab
        ...
        _tab.add(Box.createRigidArea(new Dimension(
        ...
        _itemsPanel.setPreferredSize(new Dimension(
                Constants.RESULTS_LIST_WIDTH, 1));
        ...
        _itemsPanel.setBorder(BorderFactory.createMatteBorder(0, 1, 1, 1,
                Color.BLACK));
    ...
    public void updateProbabilities() {
        ...
        for (Component c : _itemsPanel.getComponents()) {
            ...
            probabilityValue = listItem.getComparisonValue(); // getComparisonValue
                                                              // returns the
                                                              // probability for
                                                              // this room
        ...
        if (itemCount == 0)
            itemCount = 1; // avoid divide-by-zero error
        _probabilityDisplay.setProbability(probabilitySum / itemCount);
    }

    /**
     * Resizes the list. Called when the visibility of the vertical scrollbar on
     * the parent components changes.
     */
    @Override
    public void resizeItem(Dimension d) {
        // determine if components need to be resized
        boolean resize = (d.width < 0 && _fullWidth)
                || (d.width > 0 && !_fullWidth) || getHeight() != d.height;

        // resize this component
        Dimension size = this.getSize();
        if (resize) {
            this.setPreferredSize(new Dimension(size.width + d.width,
        ...
        size = _tab.getSize();
        if (resize) {
            _tab.setPreferredSize(new Dimension(size.width + d.width,
                    size.height));
        ...
        if (resize) {
            _itemsPanel.setPreferredSize(new Dimension(size.width + d.width,
                    size.height + d.height));
            ...
            // _itemsPanel.setBorder(BorderFactory.createMatteBorder(0, 1,
    ...
    public void setOpen(boolean open) {
        ...
        if (_isOpen)
            bottom = 0;
        _tab.setBorder(BorderFactory.createMatteBorder(0, 1, bottom, 1,
                Color.BLACK));
    }
    ...
    /**
     * Compares two tabs based on the average probability of their results. If
     * two tabs have the same average probability, their dorm names are compared
     * lexicographically.
     * 
     * @return a number less than 0 if this tab has a lower average probability,
     *         or if the two probabilities are equal but this tab's dorm comes
        ...
        public void mouseClicked(MouseEvent e) {
            ...
            if (open) {
                ...
                setPreferredSize(new Dimension(getWidth(),
                        Constants.RESULTS_LIST_TAB_HEIGHT
                                + _itemsPanel.getPreferredSize().height));
                revalidate();
                setOpen(open);
            } else {
                setOpen(open);
