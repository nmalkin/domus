...
public class Room implements Comparable<Room> {
    ...
    /**
     * The RoomLists that this room is a part of. Used for displaying list
     * labels on results page.
     */
    private List<RoomList> _roomLists;

    /**
     * The ResultsListItems that this room is a part of. Used for updating list
     * labels on results tab.
     */
     ...
     * 
     * we compute the probability of obtaining this room by using (externally
     * calculated) regressions coefficients and plugging values into the
     * logistic function
     */
    ...
    /**
     * Returns whether or not this room is genderNeutral. Based on Dorm's
     * genderNeutrality and Room's occupancy.
     */
    ...
    /**
     * Returns whether or not this room has an apartment fee associated with it.
     */
    ...
    public int getAverageResult() {
        ...
        for (int i = 0; i < _results.size(); i++)
            averageResult += _results.get(i).getLotteryNumber();
    ...
    /**
     * Returns the probability of receiving this room based on group's lottery
     * number.
     * 
     * To compute the probability using the "regression technique," we use the
     * coefficients from the logistic regression (set using setCoefficients) and
     * plug the lottery number into the logistic function: 1/(1+e^-(b0 + b1*x))
     * where x is the lottery number.
     * 
     * To compute the probability using the "simple technique," we compute the
     * theoretical success rate in previous years, using this number. i.e., if
     * you had this number during each of the previous years, would you have
     * gotten this room? If this technique is used, and there is no data for the
     * currently selected years, @return Constants.PROBABILITY_NO_DATA
     */
    public double getProbability() {
        if (State.getInstance().useRegressionProbability()) {
            /*
             * The regression probability technique uses pre-calculated beta
             * coefficients, and the logistic function.
             */
            return 1.0 / (1.0 + Math
                    .exp(-1
                            * (_b0_coefficient + _b1_coefficient
                                    * State.getInstance().getGroup()
                                            .getLotteryNumber())));
        } else {
            /*
             * To compute the probability using the "simple technique," we
             * compute the theoretical success rate in previous years, using
             * this number. i.e., if you had this number during each of the
             * previous years, would you have gotten this room?
             */
            Integer[] years = State.getInstance().getYears();
            int lotteryNumber = State.getInstance().getGroup()
                    .getLotteryNumber();
            int successCount = 0;

            if (_results.size() == 0) { // there are no results for the selected
                                        // years. probability concept isn't
                                        // meaningful
    ...
    public String toString() {
        ...
        for (LotteryResult r : _results)
            resultString += r.getLotteryNumber() + " ";
        resultString += "]";
    ...
    /**
     * Compares two rooms based on their probability. If two rooms have the same
     * probability, they are compared lexicographically.
     * 
     * @return a number less than 0 if this room has a lower probability, or if
     *         the two probabilities are equal but this building+room comes
