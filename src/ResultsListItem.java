...
public class ResultsListItem extends JPanel implements AccordionItem {
    ...
    private static ImageIcon _buttonIcon = new ImageIcon(Constants.ADD_FILE,
            "add to list");
    ...
    public ResultsListItem(Room room,
        ...
        Dimension size = new Dimension(Constants.RESULTS_LIST_ITEM_WIDTH,
                Constants.RESULTS_LIST_ITEM_HEIGHT);
        ...
        // _label.setFont();
        panel.add(Box.createRigidArea(new Dimension(Constants.OPEN_ICON_WIDTH,
                0)));
        panel.add(_label);
        ...
        _labelsPanel
                .setLayout(new BoxLayout(_labelsPanel, BoxLayout.LINE_AXIS));
        ...
        panel.add(Box.createRigidArea(new Dimension(
                Constants.PROBABILITY_DISPLAY_RIGHT_SPACING, 0)));

        // create button (by default, action and icon are for adding)
        // this can be changed later
        _button = new JLabel(_buttonIcon);
        _button.setPreferredSize(new Dimension(_buttonIcon.getIconWidth(),
                _buttonIcon.getIconHeight()));
        ...
        // create a panel to hold the other information (LotteryResults, GAS,
        // possible sq feet?)
        _infoPanel = new JPanel();
        size = new Dimension(Constants.RESULTS_LIST_ITEM_WIDTH,
                2 * Constants.RESULTS_LIST_ITEM_HEIGHT);
        ...
        fillerPanel.add(Box.createRigidArea(new Dimension(
                Constants.OPEN_ICON_WIDTH, 0)));
        ...
        if (_room.isGenderNeutral()) {
            ...
            genLabel.setPreferredSize(new Dimension(5,
                    Constants.RESULTS_LIST_ITEM_HEIGHT));
            genLabel.setSize(new Dimension(5,
                    Constants.RESULTS_LIST_ITEM_HEIGHT));
            size = propertiesPanel.getPreferredSize();
            propertiesPanel.setPreferredSize(new Dimension(size.width
                    + genLabel.getPreferredSize().width, size.height));
            propertiesPanel.setSize(new Dimension(size.width
                    + genLabel.getPreferredSize().width, size.height));
        }
        if (_room.hasApartmentRate()) {
            JLabel rateLabel = new JLabel("A ");
            propertiesPanel.add(rateLabel);
            rateLabel.setPreferredSize(new Dimension(5,
                    Constants.RESULTS_LIST_ITEM_HEIGHT));
            rateLabel.setSize(new Dimension(5,
                    Constants.RESULTS_LIST_ITEM_HEIGHT));
            size = propertiesPanel.getPreferredSize();
            propertiesPanel.setPreferredSize(new Dimension(size.width
                    + rateLabel.getPreferredSize().width, size.height));
            propertiesPanel.setSize(new Dimension(size.width
                    + rateLabel.getPreferredSize().width, size.height));
        }
        if (_room.getDorm().isSophomoreOnly()) {
            ...
            sophLabel.setPreferredSize(new Dimension(5,
                    Constants.RESULTS_LIST_ITEM_HEIGHT));
            sophLabel.setSize(new Dimension(5,
                    Constants.RESULTS_LIST_ITEM_HEIGHT));
            size = propertiesPanel.getPreferredSize();
            propertiesPanel.setPreferredSize(new Dimension(size.width
                    + sophLabel.getPreferredSize().width, size.height));
            propertiesPanel.setSize(new Dimension(size.width
                    + sophLabel.getPreferredSize().width, size.height));
        }
        JLabel results = new JLabel("Past Results:");
        propertiesPanel.add(results);
        propertiesPanel.setPreferredSize(new Dimension(30,
                Constants.RESULTS_LIST_ITEM_HEIGHT));
        propertiesPanel.setSize(new Dimension(30,
                Constants.RESULTS_LIST_ITEM_HEIGHT));
        size = propertiesPanel.getPreferredSize();
        propertiesPanel.setPreferredSize(new Dimension(size.width
                + results.getPreferredSize().width, size.height));
        propertiesPanel.setSize(new Dimension(size.width
                + results.getPreferredSize().width, size.height));
        ...
        pastResultsPanel.setLayout(new BoxLayout(pastResultsPanel,
                BoxLayout.LINE_AXIS));
        size = new Dimension(0, 2 * Constants.RESULTS_LIST_ITEM_HEIGHT);
        ...
        for (LotteryResult lotteryResult : _room.getResults()) {
            ...
            pastResult
                    .setLayout(new BoxLayout(pastResult, BoxLayout.PAGE_AXIS));
            int left = 1;
            if (count == 0)
                left = 0;
            pastResult.setBorder(BorderFactory.createMatteBorder(0, left, 0, 0,
                    Color.BLACK));
            ...
            int width = Math.max(year.getPreferredSize().width, result
                    .getSize().width);
            ...
            size = new Dimension(size.width + pastResult.getSize().width,
                    2 * Constants.RESULTS_LIST_ITEM_HEIGHT);
            pastResultsPanel.setPreferredSize(size);
            pastResultsPanel.setSize(size);
            count++;
        ...
        holderPanel.setBorder(BorderFactory.createMatteBorder(0, 1, 1, 0,
                Color.BLACK));
    ...
    /**
     * Returns the parent list associated with this item. May be null.
     */
    ...
    /**
     * ResultsListItems cannot be open, so instead this sets the selection state
     * of this item. It bolds the item's label if it is selected.
     */
    @Override
    public void setOpen(boolean open) {
        ...
        if (open) {
            ...
            _label.setBorder(BorderFactory.createMatteBorder(0, 1, 0, 0,
                    Color.BLACK));
        } else {
            _infoPanel.setVisible(false);
            _label.setBorder(BorderFactory.createMatteBorder(0, 0, 0, 0,
                    Color.BLACK));
            height = -height;
        }
    ...
    public void resizeItem(Dimension d) {
        ...
        boolean resize = (d.width < 0 && _fullWidth)
                || (d.width > 0 && !_fullWidth);
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            for (ResultsListItem rli : getRoom().getListItems()) {
                ...
                if (rli.getParentList() != null) {
                    ResultsListTab rlt = (ResultsListTab) rli.getParent()
                            .getParent();
