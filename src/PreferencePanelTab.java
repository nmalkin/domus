...
public class PreferencePanelTab extends JPanel implements AccordionItem {
    ...
    PreferencePanelTab(House house,
            AccordionList<PreferencePanelTab, PreferencePanelItem> list) {
        super();
        this.setLayout(null);
        ...
        for (SubGroup sg : house) {
            for (Person p : sg) {
                JLabel person = new JLabel(new ImageIcon(p.getGender()
                        .getImage().getScaledInstance(-1, 30,
                                Image.SCALE_SMOOTH)));
                if (!p.getName().equals("A Person"))
                    person.setToolTipText(p.getName());
                _tab.add(person);
