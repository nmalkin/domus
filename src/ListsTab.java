...
public class ListsTab extends JPanel implements Runnable {
    ...
    private ListsTab() {
        ...
        this.setPreferredSize(new Dimension(Constants.LISTS_PANEL_WIDTH,
                Constants.LISTS_HEIGHT));

        // create a panel for the instructions which will be displayed whenever
        // there are no lists
        _instructionsLabel = new JLabel(Constants.LISTS_INSTRUCTIONS);
        _instructionsLabel.setPreferredSize(new Dimension(
                Constants.LISTS_INSTRUCTIONS_WIDTH,
                Constants.LISTS_INSTRUCTIONS_HEIGHT));
        ...
        _listsPanel.setPreferredSize(new Dimension(_instructionsLabel
                .getPreferredSize().width, Constants.LISTS_HEIGHT));
        _listsPanel.setSize(new Dimension(
                _instructionsLabel.getPreferredSize().width,
                Constants.LISTS_HEIGHT));

        // create a scroll pane to hold the panel of lists
        _scroller = new JScrollPane(_listsPanel,
                JScrollPane.VERTICAL_SCROLLBAR_NEVER,
                JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
        _scroller.setPreferredSize(new Dimension(_listsPanel.getSize().width,
                Constants.LISTS_HEIGHT));
        _scroller.setViewportBorder(new EmptyBorder(0, 0, 0, 0));
        _scroller.getHorizontalScrollBar().setBorder(
                new EmptyBorder(0, 0, 0, 0));

        // create and add the scroll buttons which will be visible when
        // necessary
        ...
        // add a GroupStateChangeListener to the Group for lotteryNumber
        // differences
        State.getInstance().getGroup().addGroupStateChangeListener(
                new GroupStateChangeListener());
    }
        ...
        for (RoomList rl : State.getInstance().getRoomLists()) {
            if (i >= _lists.size()) {
                ...
                if (i == 0) {
                    ...
                    _listsPanel.setPreferredSize(new Dimension(0,
                            Constants.LISTS_HEIGHT));
                    _listsPanel
                            .setSize(new Dimension(0, Constants.LISTS_HEIGHT));
                    _scroller.setPreferredSize(new Dimension(0,
                            Constants.LISTS_HEIGHT));
                    _scroller.setPreferredSize(new Dimension(0,
                            Constants.LISTS_HEIGHT));
                }

                // create a new list and add it to the lists panel, as well as a
                // spacing component
                ...
                int extra = 0;
                if (i != _lists.size()) {
                    _listsPanel.add(Box.createRigidArea(new Dimension(
                            Constants.LISTS_HORIZONTAL_GAP, 0)));
                    extra = Constants.LISTS_HORIZONTAL_GAP;
                }

                // reset the size of the lists panel
                Dimension size = _listsPanel.getPreferredSize();
                _listsPanel.setPreferredSize(new Dimension(size.width
                        + Constants.LISTS_WIDTH + extra, size.height));
                _listsPanel.setSize(new Dimension(size.width
                        + Constants.LISTS_WIDTH + extra, size.height));

                // reset the size of the scroll pane if needed
                if (i < Constants.LISTS_DISPLAYED) {
                    size = _scroller.getPreferredSize();
                    _scroller.setPreferredSize(new Dimension(size.width
                            + Constants.LISTS_WIDTH + extra, size.height));
                    _scroller.setSize(new Dimension(size.width
                            + Constants.LISTS_WIDTH + extra, size.height));
                }
            }
            i++;
    ...
    void removeList(ListPanel list) {
        ...
        _listsPanel.setPreferredSize(new Dimension(size.width
                - Constants.LISTS_WIDTH - extra, size.height));
        _listsPanel.setSize(new Dimension(size.width - Constants.LISTS_WIDTH
                - extra, size.height));
        ...
        if (_lists.size() < Constants.LISTS_DISPLAYED) {
            ...
            _scroller.setPreferredSize(new Dimension(size.width
                    - Constants.LISTS_WIDTH - extra, size.height));
            _scroller.setSize(new Dimension(size.width - Constants.LISTS_WIDTH
                    - extra, size.height));
        }
        ...
        if (_lists.size() == 0) {
            ...
            _listsPanel.setPreferredSize(new Dimension(_instructionsLabel
                    .getPreferredSize().width, Constants.LISTS_HEIGHT));
            _listsPanel.setSize(new Dimension(_instructionsLabel
                    .getPreferredSize().width, Constants.LISTS_HEIGHT));
            _scroller.setPreferredSize(new Dimension(
                    _listsPanel.getSize().width, Constants.LISTS_HEIGHT));
        }
    ...
    /**
     * Sets the visibility of the scroll buttons. They are made visible if there
     * are more than LISTS_DISPLAYED lists. Otherwise, they are hidden.
     */
        ...
        while (_scrollLeft || _scrollRight) {
            ...
            if (_scrollRight) {
                ...
                if (r.x + r.width + _scrollWidth >= _listsPanel.getWidth()) {
                    view.setViewPosition(new Point(_listsPanel.getWidth()
                            - r.width, r.y));
                    _scrollRight = false;
                    _scrollLeft = false;
                    break;
